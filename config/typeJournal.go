...
type Journal struct {
	...
	//
	//Enable or disable the durability journal to ensure data files remain valid and
	//recoverable.
	...
	//storage.journal.enabled: false for replica set members that use the WiredTiger
	//storage engine.
	Enabled LogicBoolean `yaml:"enabled"`

	//Default: 100
	//
	//The maximum amount of time in milliseconds that the mongod process allows between
	//journal operations. Values can range from 1 to 500 milliseconds. Lower values
	...
	//a write that includes or implies j:true will cause an immediate sync of the journal.
	//For details or additional conditions that affect the frequency of the sync, see
	//Journaling Process.
	//
